...
def ensure_web_template(base_url, headers):
	...
	resp = requests.get(
		f"{base_url}/api/resource/Web%20Template/Raw HTML Section", headers=headers, timeout=10
	)
	...
	resp = requests.post(
		...
		json={
			...
			"fields": [{"fieldname": "html_content", "fieldtype": "Text", "label": "HTML Content"}],
		},
		timeout=15,
	)
	if resp.status_code == 200:
...
def deploy_page(base_url, headers, title, route, html_content, css):
	...
	if existing:
		...
		resp = requests.post(f"{base_url}/api/resource/Web%20Page", headers=headers, json=data, timeout=30)
		if resp.status_code == 200:
