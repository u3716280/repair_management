...
# v3 drops the "Remaining" / "Usage %" cards v2 kept alongside "Message Quota"
# -- the used/limit string on "Message Quota" already conveys both, so the
# extra cards were redundant. Only one card per channel remains.
CARD_SPECS = (("Message Quota", "quota_usage_display", "#2490EF"),)

# Card labels from v2 that are retired by this patch.
RETIRED_LABELS = ("Remaining", "Usage %")
...
def _retire_old_cards(channels):
    """Delete the v2 per-channel cards that this patch drops."""
    removed = []
    for channel in channels:
...
def apply():
    ...
    result = {
        ...
        "note": "Only the Message Quota (used/limit) card remains per channel; Remaining and Usage % were retired.",
    }
