...
def cleanup_legacy_doctypes():
	...
	# delete_doc does not remove Custom DocPerm rows; orphans break File permission queries
	# (DoesNotExistError in File list / Library picker), so clear them even if already absent.
	orphan_perms = frappe.get_all(
		"Custom DocPerm",
		filters={"parent": ["in", LEGACY_DOCTYPES]},
		pluck="name",
	)
	if orphan_perms:
		frappe.db.delete("Custom DocPerm", {"name": ["in", orphan_perms]})
		frappe.clear_cache()
	...
	return _print(
		{
			"removed_legacy_doctypes": removed,
			"already_absent": missing,
			"removed_custom_docperms": len(orphan_perms),
		}
	)
...
@frappe.whitelist()
def verify():
	legacy_present = [doctype for doctype in LEGACY_DOCTYPES if frappe.db.exists("DocType", doctype)]
	missing_expected = [doctype for doctype in EXPECTED_DOCTYPES if not frappe.db.exists("DocType", doctype)]

	doctype_root = Path(frappe.get_app_path("repair_management", "repair_management", "doctype"))
	...
	legacy_custom_docperms = frappe.get_all(
		"Custom DocPerm",
		filters={"parent": ["in", LEGACY_DOCTYPES]},
		pluck="parent",
		distinct=True,
	...
	result = {
		...
		"legacy_custom_docperms_present": legacy_custom_docperms,
		...
		and not legacy_custom_docperms
