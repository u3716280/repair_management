...
from repair_management.integrations.line.flows.base import context, create_session, set_context
	...
	return {
		...
		"body": {
			...
			"contents": [
				...
				{
					"type": "text",
					"text": item.get("subtitle") or "Sales Order",
					"size": "sm",
					"color": "#555555",
					"wrap": True,
				},
	...
	return {
		...
		"body": {
			...
			"contents": [
				...
				{
					"type": "text",
					"text": row.item_name or row.item_code or "-",
					"size": "sm",
					"color": "#555555",
					"wrap": True,
				},
...
def start(channel, user_id, reply_token, flow, **kwargs):
	...
	if not rows:
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": f"ไม่พบ {p.target_doctype} ที่อยู่ระหว่างดำเนินการ"}]
		)
		return
...
def show_page(channel, user_id, reply_token, params, **kwargs):
	...
	if not session:
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": "รายการนี้หมดอายุแล้ว กรุณาเริ่มรายการใหม่"}]
		)
		return
...
def select_item(channel, user_id, reply_token, params, **kwargs):
	...
	if not document_search.is_eligible(p, session.target_document):
		session.db_set({"status": "Cancelled", "current_state": "Cancelled"})
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": "Sales Order ไม่อยู่ในสถานะที่อนุญาตแล้ว กรุณาเริ่มรายการใหม่"}]
		)
		return
...
def select_burn_in(channel, user_id, reply_token, params, **kwargs):
	...
	if reason:
		session.db_set({"status": "Cancelled", "current_state": "Cancelled"})
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": "เอกสารหรือรายการสินค้ามีการเปลี่ยนแปลง กรุณาเริ่มรายการใหม่"}]
		)
		return
			...
			[
				...
				{"type": "action", "action": {"type": "cameraRoll", "label": "เลือกเพิ่ม"}},
				{
					...
					"action": {
						"type": "postback",
						"label": "เสร็จสิ้น",
						"displayText": "เสร็จสิ้นการส่งรูป",
						"data": _postback("media_finish", session=session.name),
					},
				},
			]
		)
			...
			[
				...
				},
				{
					...
					"action": {
						"type": "postback",
						"label": "เสร็จสิ้น",
						"displayText": "เสร็จสิ้นการส่งวิดีโอ",
						"data": _postback("media_finish", session=session.name),
					},
				},
			]
...
def _lock_session(session):
	"""Row-lock the session and refresh the fields concurrent jobs race on.

	Each image in a multi-photo send arrives as its own webhook event, and with
	several short-queue workers those receive() jobs (plus finish() and the
	long-queue download() jobs) run in parallel. A plain session.reload() reads
	the transaction's snapshot, so two jobs could both read accepted_media=N and
	both write N+1, or a late writer could drop finish_requested/finish_expected
	-- leaving the session stuck in Waiting Media with no collage. A locking
	read returns the latest committed row and blocks other writers until this
	job commits.
	"""
	rows = frappe.db.sql(
		"""select status, current_state, context_json
        from `tabLINE Flow Session` where name = %s for update""",
		session.name,
		as_dict=True,
	)
	if rows:
...
def _downloaded_image_count(session):
	# Locking read so a download committed after this transaction's snapshot
	# is still counted (see _lock_session).
	return frappe.db.sql(
		"""select count(*) from `tabLINE Media File`
        where flow_session = %s and media_type = 'Image'
        and processing_status in ('Downloaded', 'Finalized')
        lock in share mode""",
		session.name,
	)[0][0]
...
def _try_complete_media_session(channel, session, p, expected_count, reply_token=None):
	...
	with frappe.cache.lock(lock_key, timeout=60, blocking_timeout=15):
		...
		if p.media_type == "Image":
			...
			frappe.enqueue(
				finalize, queue="long", channel=channel, session_name=session.name, enqueue_after_commit=True
			)
			return True
...
def finish(channel, user_id, reply_token, params, **kwargs):
	...
	if accepted < minimum:
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": f"กรุณาส่งอย่างน้อย {minimum} {unit}ก่อนกดเสร็จสิ้น"}]
		)
		return
	if accepted > maximum:
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": f"จำนวนไฟล์เกินกำหนดสูงสุด {maximum} {unit}"}]
		)
		return
...
def receive(channel, user_id, reply_token, session, message, **kwargs):
	...
	if accepted >= maximum:
		LineClient(channel).reply(
			reply_token, [{"type": "text", "text": f"รับไฟล์ครบจำนวนสูงสุด {maximum} ไฟล์แล้ว"}]
		)
		return True
...
def download(channel, session_name, message):
		...
		session.db_set(
			{
				...
				"error_message": "Target document or item is no longer valid",
			}
		)
	...
	extension = mimetypes.guess_extension(content_type.split(";")[0]) or (
		".jpg" if p.media_type == "Image" else ".mp4"
	)
...
def finalize(channel, session_name):
	...
	if not session.target_doctype or not session.target_document:
		session.db_set(
			{"status": "Failed", "current_state": "Failed", "error_message": "Missing target document"}
		)
		LineClient(channel).push(
			session.line_user_id,
		...
		session.db_set(
			{
				"status": "Failed",
				"current_state": "Failed",
				"error_message": "Target document or item is no longer eligible",
			}
		)
		LineClient(channel).push(
			session.line_user_id, [{"type": "text", "text": "เอกสารหรือรายการสินค้าเปลี่ยนแปลง จึงยังไม่ได้แนบรูป"}]
		)
		return
	...
	try:
		...
		if existing:
			cleanup_errors = (
				_cleanup_original_media(rows) if len(rows) > 1 or int(ctx.get("burn_in") or 0) else []
			)
		...
		if len(rows) == 1 and p.single_image_mode == "Attach Directly":
			...
			if not burn_enabled:
				...
				if not _verify_attachment(
					media_row.temporary_file, session.target_doctype, session.target_document
				):
		...
		if missing:
			...
			if recovered and _verify_attachment(
				recovered.name, session.target_doctype, session.target_document
			):
				...
				return
			frappe.throw(
				"Missing original image files and no recoverable collage was found: " + ", ".join(missing)
			)
