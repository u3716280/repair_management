...
def _get_authorized_identity(line_user_id: str) -> None:
    """Authorization is now purely person-level: LINE Recipient is one row
    per physical LINE user (see services/recipient.py), so there is no more
    per-channel duplication to resolve here.
    """
    identity = frappe.db.get_value(
        "LINE Recipient",
        line_user_id,
        ["enabled", "allow_mark_attendance"],
        as_dict=True,
    )
    if not identity or not identity.enabled or not identity.allow_mark_attendance:
...
def authenticate(id_token: str, channel_name: str) -> AttendanceAuthContext:
    ...
    _get_authorized_identity(line_user_id)
