...
class ReorderPointCalculator {
	...
	set_defaults() {
		...
		this.percentile_field.set_value("95");
		this.warehouse_field.set_value("เอกไทย - ET");
	}
	...
	make_filters() {
		...
		this.warehouse_field = this.make_control(filter_row, "col-md-3", {
			...
			get_query: () => ({
				filters: {
					name: ["in", ["เอกไทย - ET", "บ้าน 147 - ET", "บ้าน 88 - ET"]],
				},
			}),
		});
		...
		this.percentile_field = this.make_control(action_row, "col-md-3", {
			fieldtype: "Select",
			fieldname: "percentile",
			label: "Percentile (Safety Stock)",
			options: "90\n95\n97.5\n99",
			description:
				"P90: หาซื้อง่าย ไม่สำคัญ • P95: สินค้าทั่วไป • P97.5: กระทบการผลิต • P99: สำคัญมาก/Lead Time นาน",
		});
		...
		this.make_percentile_guide();
	}

	make_percentile_guide() {
		const rows = [
			["P90", "90%", "หาซื้อง่าย ไม่สำคัญ"],
			["P95", "95%", "สินค้าทั่วไป"],
			["P97.5", "97.5%", "กระทบการผลิต"],
			["P99", "99%", "สำคัญมากหรือ Lead Time นาน"],
		];
		const table_rows = rows
			.map(
				([p, sl, desc]) =>
					`<tr><td style="padding:2px 12px 2px 0;"><b>${p}</b></td><td style="padding:2px 12px 2px 0;">${sl}</td><td style="padding:2px 0;">${desc}</td></tr>`
			)
			.join("");

		$(
			`<div style="margin-bottom: 15px; font-size: 12px; color: var(--text-muted);">
				<div style="margin-bottom: 4px;">แนวทางเลือก Percentile ตามความสำคัญของสินค้า:</div>
				<table style="border-collapse: collapse;"><tbody>${table_rows}</tbody></table>
			</div>`
		...
		frappe
			.call({
				...
				args: {
					...
					order_cycle_days,
					percentile: Number(this.percentile_field.get_value() || 95),
				},
			})
			.then((r) => {
				frappe.dom.unfreeze();
	...
	render_table(data) {
		...
		const columns = [
			...
			{ name: "Events", editable: false, width: 75 },
