...
def before_cancel_payment_entry(doc, method=None) -> None:
    if doc.custom_additional_salary:
        ...
        # Salary are still submitted and link back via custom_payment_entry.
...
def on_trash_payment_entry(doc, method=None) -> None:
    if not doc.custom_additional_salary or not frappe.db.exists(
        "Additional Salary", doc.custom_additional_salary
    ):
        return
